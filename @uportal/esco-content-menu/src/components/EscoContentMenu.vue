  ...
  export default {
    ...
    data() {
      return {
        favorites: [],
        portlets: [],
        backgroundImg: 'https://lycees.netocentre.fr//annuaire_images/logos/77534903800072/logoportail1.jpg?1',
        isSmall: false
      }
    },
    mounted() {
    ...
    methods: {
      ...
      fetchFavorites() {
        if (process.env.NODE_ENV === "development") {
          this.favorites.push("pbookmarks", "snooper", "google-translate-portlet");
          ...
          oidc()
            .then(token => {
              ...
              fetch(
                ...
                .then(parseJSON)
                .then(data => {
                  if (data && data.authenticated && data.layout && data.layout.globals && data.layout.globals.hasFavorites) {
