    ...
    methods: {
      handleSubstrSentence () {
        ...
        if (stNodeHeight > maxHeight) {
          ...
          const stNodeDelStr = stNode.innerHTML.substring(stNodeLen - endLen, stNodeLen).trim();
          const match = stNodeDelStr.match(/\s+/g);
          const extraLen = match && match.length ? match.length : 0;
          const newHtml = stNode.innerHTML.substring(0, stNodeLen - endLen - extraLen).trim() + this.endChar + this.endHtml;
          stNode.innerHTML = newHtml;
        }
      }
    },
